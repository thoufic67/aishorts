...
export function VideoFramesPanel({
  ...
  const handleEdit = (index: number, segment: VideoSegment) => {
    console.log("thoufic edit", { index, segment });
    setEditingState({
      index,
      mode: "image",
  ...
  return (
    <div className={isHorizontal ? "w-full" : "w-80 border-r bg-white"}>
      ...
      {showHeader && (
        <div
          className={`flex items-center justify-between ${isHorizontal ? "border-b p-4" : "h-14 border-b px-4"}`}
        >
      ...
      {/* Frames list */}
      <div
        className={`${isHorizontal ? "overflow-x-auto p-4" : "flex-1 overflow-y-auto p-4"}`}
      >
        <div className={isHorizontal ? "flex gap-3" : "space-y-3"}>
          {segments.map((segment, index) => (
            <div
              ...
              className={
                isHorizontal ? "flex flex-shrink-0 items-center gap-2" : ""
              }
            >
              {/* Add frame button before first frame */}
              {index === 0 && (
                <div
                  className={`flex ${isHorizontal ? "flex-col items-center" : "mb-3 justify-center"}`}
                >
                  <Button
                    variant="ghost"
                    size="sm"
              ...
              <Card
                className={`group cursor-pointer border transition-all ${isHorizontal ? "relative w-20" : ""} ${
              ...
              >
                <div className={isHorizontal ? "p-2" : "p-3"}>
                  ...
                  {!isHorizontal && (
                    <div className="mb-2 flex items-center justify-between">
                      <div className="flex w-full items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <Dialog
                            open={editingState?.index === index}
                            onOpenChange={(open) =>
                              !open && setEditingState(null)
                            }
                          >
                                          ...
                                          <div className="space-y-4">
                                            ...
                                            <div>
                                              ...
                                              {editingState?.script !==
                                                currentSegment.text && (
                                                <div className="mt-1 text-xs text-blue-600">
                                                  ⚡ Script modified - click
                                                  "Regenerate Audio" to apply
                                                  changes
                                                </div>
                                              )}
                                            </div>
                  ...
                  <div className={isHorizontal ? "relative" : "space-y-2"}>
                    ...
                    <div
                      className={`relative overflow-hidden rounded ${isHorizontal ? "mx-auto h-16 w-12" : "mx-auto"}`}
                    >
                      ...
                      {segment.media && segment.media.length > 0 ? (
                        ...
                        /* Fallback indicator */
                        <div
                          className={`flex items-center justify-center ${isHorizontal ? "h-full w-full bg-gray-200 text-gray-400" : "absolute inset-0"}`}
                        >
                          {isHorizontal ? (
                      ...
                      <div
                        className={`absolute ${isHorizontal ? "bottom-1 right-1" : "bottom-2 right-2"}`}
                      >
                        <div
                          className={`rounded bg-black/50 font-medium text-white ${isHorizontal ? "px-1 py-0.5 text-xs" : "px-1.5 py-0.5 text-xs"}`}
                        >
                          {isHorizontal
      ...
      <Dialog
        open={!!newFrameState}
        onOpenChange={(open) => !open && setNewFrameState(null)}
      >
                  ...
                  {voiceOptions.slice(0, 3).map((voice) => (
                    <div key={voice.id} className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={(e) =>
                          setNewFrameState((prev) =>
                            prev ? { ...prev, voice: e.target.value } : null,
                          )
                        }
                        className="h-4 w-4 text-blue-600"
                      />
                    ...
                    <div key={model.id} className="flex items-center space-x-2">
                      <input
                        ...
                        onChange={(e) =>
                          setNewFrameState((prev) =>
                            prev
                              ? { ...prev, imageModel: e.target.value }
                              : null,
                          )
                        }
                        className="h-4 w-4 text-blue-600"
